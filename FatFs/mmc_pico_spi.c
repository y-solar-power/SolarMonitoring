/*------------------------------------------------------------------------*/
/* Raspberry Pi Pico : MMCv3/SDv1/SDv2 (SPI mode) control module                   */
/*------------------------------------------------------------------------*/
/*
...
/*
   GPIO 12 MISO/spi1_rx-> SDO
   GPIO 13 Chip select -> !CS
   GPIO 14 SCK/spi1_sclk -> SCK
   GPIO 15 MOSI/spi1_tx -> SDI
*/
#define DEF_SPI_TX_PIN  11
#define DEF_SPI_RX_PIN  12
#define DEF_SPI_SCK_PIN 10
#define DEF_SPI_CSN_PIN 13
...
bool interval_1ms(struct repeating_timer *t){
	disk_timerproc();
	return true;
}
char interval_activated = 0;
static void init_interval(void){
	if(interval_activated) return;
	interval_activated = 1;
	static repeating_timer_t timer;
	add_repeating_timer_ms( -1, &interval_1ms, NULL, &timer );
}
